#!/usr/bin/env python3
"""
00_n2v_temporal_two_phase.py
---------------------------------
TGNN-temporal (Node2Vec fusion) two-phase HPO orchestrator.
...
Artifacts layout under --artifacts-root/n2v_temporal/<tag>:
- phaseA/ (Optuna study + results)
- finalize/ (robust summaries + robust_best_params.json)

Usage example:
python src/n2v_temporal/00_tgnn_hpo_two_phase.py \
  ...
  --n2v-dir artifacts/node2vec/core_v1 \
  --version-tag n2v_temporal_v1 --trials-a 40 --epochs-a 8 --top-k 3 --epochs-finalize 60
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Two-phase HPO orchestrator for TGNN-temporal with Node2Vec fusion"
    )
    # Data
    ...
    # Layout / versioning
    p.add_argument("--version-tag", type=str, default="n2v_temporal_v1")
    p.add_argument("--artifacts-root", type=str, default="artifacts/n2v_temporal")
    p.add_argument("--results-root", type=str, default="results/n2v_temporal")
...
def main():
    ...
    print("=" * 70)
    print("TGNN-Temporal+Node2Vec Two-Phase HPO Orchestrator")
    print("=" * 70)
    ...
    cmd_a = [
        sys.executable,
        "src/n2v_temporal/01_tgnn_hpo_optuna.py",
        "--adj",
        args.adj,
        "--features",
        args.features,
    ...
    cmd_finalize = [
        sys.executable,
        "src/n2v_temporal/02_tgnn_finalize_hpo.py",
        "--adj",
        args.adj,
        "--features",
        args.features,
    ...
    print("python src/n2v_temporal/03_n2v_temporal_train_eval.py \\")
    ...
    print(f"  --n2v-dir {args.n2v_dir} \\")
    print(f"  --best-params-path {best_params_path} \\")
    print(f"  --multi-seed --seeds {args.seeds} \\")
    print(f"  --out-dir results/n2v_temporal/{args.version_tag}_production \\")
