#!/usr/bin/env python3
"""Appendix A.3b full structural slices (Validation)."""

# Table CLI cheat sheet: python src/analysis/chapter2/tab_06_structural_slices_val.py \
#   --meta-val <val.parquet> --meta-test <test.parquet> \
#   --scores-val <val_scores.parquet> --scores-test <test_scores.parquet> \
#   --output tables/chapter2/tab_06_structural_slices_val.tex
...
def render_table(assets: ScorecardAssets, output_path: Path) -> None:
    summary = assets.summary_by_split.get("validation")
    if summary is None:
        raise RuntimeError("Validation summary unavailable for structural slices")
    ...
    lines = [
        "% Auto-generated full structural slice table (Validation)",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Appendix A.3b structural slices (Validation)")
    parser.add_argument(
        "--meta-val",
    ...
    parser.add_argument(
        "--output",
        default="tables/chapter2/tab_06_structural_slices_val.tex",
