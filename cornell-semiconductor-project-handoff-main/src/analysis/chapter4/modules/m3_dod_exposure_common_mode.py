#!/usr/bin/env python3
"""Module 3: DoD exposure and common-mode dependence by graph view."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Chapter 4 Module 3 DoD exposure/common-mode")
    parser.add_argument(
        "--config",
...
def load_prime_weights(
    ...
    m3_cfg = cfg.get("m3", {})
    mode = str(m3_cfg.get("prime_weights_mode", "unit")).strip().lower()
...
def compute_reach_bfs_capped(
    ...
    up_count = np.zeros(n_primes, dtype=np.int64)
    up_semi_count = np.zeros(n_primes, dtype=np.int64)
    up_prime_count = np.zeros(n_primes, dtype=np.int64)
    ...
    return node_prime_count, node_prime_weight, up_count, up_semi_count, up_prime_count
...
def compute_reach_full_scc(
    ...
    # Deduplicate directed edges for stable SCC and condensation.
    ...
    return node_prime_count, node_prime_weight, up_count, up_semi_count, up_prime_count, diag
...
def main() -> None:
    ...
    prime_weights, prime_weight_source, prime_weight_diag = load_prime_weights(cfg, prime_uids)
    ...
    m3_cfg = cfg.get("m3", {})
    hop_cap_raw = m3_cfg.get("hop_cap", None)
    if hop_cap_raw is None or str(hop_cap_raw).strip().lower() in {"full", "none", "null", ""}:
        ...
        if hop_cap < 0:
            raise ValueError("m3.hop_cap must be >= 0 when capped")
    exclude_self = bool(m3_cfg.get("exclude_self_from_upstream", True))
    top_n = int(m3_cfg.get("top_n_export", 100))
    top_n_nonprime = int(m3_cfg.get("export_nonprime_top_n", top_n))

    prime_outputs: dict[str, str] = {}
    node_outputs: dict[str, str] = {}
    common_outputs: dict[str, str] = {}
    common_nonprime_outputs: dict[str, str] = {}
    ...
    for view_name, view_spec in views.items():
        ...
        common_nonprime = (
            node_df.loc[~node_df["is_tier1_prime"]]
            .sort_values(
                ["prime_reach_weighted", "prime_reach_count", "analysis_uid"],
                ascending=[False, False, True],
            )
        ...
        prime_out = m3_dir / f"prime_upstream_sizes_{view_name}.csv"
        node_out = m3_dir / f"node_prime_reach_{view_name}.parquet"
        common_out = m3_dir / f"common_mode_top100_{view_name}.csv"
        common_nonprime_out = m3_dir / f"common_mode_top100_nonprime_{view_name}.csv"

        prime_df.to_csv(prime_out, index=False)
        node_df.to_parquet(node_out, index=False)
        common_all.to_csv(common_out, index=False)
        common_nonprime.to_csv(common_nonprime_out, index=False)

        prime_outputs[view_name] = str(prime_out)
        node_outputs[view_name] = str(node_out)
        common_outputs[view_name] = str(common_out)
        common_nonprime_outputs[view_name] = str(common_nonprime_out)

        # Reconciliation identity:
        # sum(node_prime_count) == sum(up_count) + n_primes (if self excluded), else == sum(up_count)
    ...
    run_metadata = {
        ...
        "settings": {
            ...
            "top_n_export": top_n,
            "top_n_nonprime_export": top_n_nonprime,
        },
        "view_stats": view_stats,
    }
    run_metadata_out = m3_dir / "run_metadata.json"
    ...
    manifest = {
        ...
        "outputs": {
            "prime_upstream_sizes": prime_outputs,
            "node_prime_reach": node_outputs,
            "common_mode_top100": common_outputs,
            "common_mode_top100_nonprime": common_nonprime_outputs,
            "run_metadata": str(run_metadata_out),
        },
    }
    manifest_out = m3_dir / "manifest_m3.json"
