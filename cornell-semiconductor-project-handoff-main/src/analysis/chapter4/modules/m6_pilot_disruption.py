#!/usr/bin/env python3
"""Module 6 pilot: disruption severity screening on the full view."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Chapter 4 Module 6 pilot disruption analysis")
    parser.add_argument(
        "--config",
...
def main() -> None:
    ...
    m6_dir = out_root / snapshot / "m6_pilot"
    ...
    if len(prime_indices) == 0:
        raise ValueError("No primes found for pilot")
    if len(semi_indices) == 0:
        raise ValueError("No strict semis found for pilot")
    ...
    for idx, node_idx in enumerate(candidate_indices.tolist(), start=1):
        ...
        if idx % 50 == 0:
            print(f"[m6-pilot] H1 screened {idx}/{len(candidate_indices)}")
    h1_df = (
    ...
    h1_out = m6_dir / f"node_single_removal_impacts_h1_{primary_view}.csv"
    ...
    for idx, node_idx in enumerate(deep_indices.tolist(), start=1):
        ...
        if idx % 25 == 0:
            print(f"[m6-pilot] deep eval {idx}/{len(deep_indices)}")
    deep_df = (
    ...
    deep_out = m6_dir / f"node_single_removal_impacts_deep_{primary_view}.csv"
    ...
    for frac in removal_fracs:
        ...
        for rep in range(1, random_repeats + 1):
            ...
            if rep % 5 == 0:
                print(f"[m6-pilot] random robustness frac={frac:.4f} repeat={rep}/{random_repeats}")
    ...
    def greedy_interdiction(objective: str, k_value: int) -> list[dict[str, Any]]:
        ...
        for step in range(1, k_value + 1):
            ...
            print(
                f"[m6-pilot] interdiction objective={objective} k={k_value} step={step}/{k_value}"
            )
        return rows
    ...
    manifest = {
        ...
        "outputs": {
            ...
            "single_node_h1": str(h1_out),
            "single_node_deep": str(deep_out),
