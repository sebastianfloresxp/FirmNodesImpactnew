#!/usr/bin/env python3
"""Module 6.1: weighted disruption severity analysis."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Chapter 4 Module 6.1 weighted disruption analysis"
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--view",
        default=None,
        help="Optional override for config m6_1.primary_view (e.g., disclosed, observed, full)",
    )
...
def load_prime_weight_table(
    ...
    if uid_col is None:
        diagnostics["fallback_to_unit"] = True
        diagnostics["missing_columns"] = {"uid_col": None}
        base = pd.DataFrame(
            {
                "analysis_uid": prime_uids.astype(str),
                "weight_unit": np.ones(len(prime_uids), dtype=np.float64),
    ...
    merged["weight_unit"] = (
        pd.to_numeric(merged["weight_unit"], errors="coerce").fillna(1.0).astype(np.float64)
    )
    merged["weight_log_obligation"] = (
        pd.to_numeric(merged["weight_log_obligation"], errors="coerce")
        .fillna(0.0)
        .astype(np.float64)
    )
    merged["weight_raw_obligation"] = (
        pd.to_numeric(merged["weight_raw_obligation"], errors="coerce")
        .fillna(0.0)
        .astype(np.float64)
    )

    prime_set = set(prime_uids.astype(str).tolist())
    matched = int(sum(uid in set(df["analysis_uid"].astype(str).tolist()) for uid in prime_set))
    diagnostics["matched_primes"] = matched
    diagnostics["missing_primes"] = int(len(prime_set) - matched)
...
def parse_h1_profiles(m6_cfg: dict[str, Any]) -> list[dict[str, str]]:
    profiles = m6_cfg.get("h1_profiles")
    if isinstance(profiles, list) and len(profiles) > 0:
        parsed: list[dict[str, str]] = []
        for p in profiles:
            if isinstance(p, dict):
                w = str(p.get("weight_mode", "")).strip().lower()
                f = str(p.get("support_form", "")).strip().lower()
                if w and f:
                    parsed.append({"weight_mode": w, "support_form": f})
        if parsed:
            return parsed
    return [
        {"weight_mode": "unit", "support_form": "support_count"},
        {"weight_mode": "log_obligation", "support_form": "any_support"},
        {"weight_mode": "raw_obligation", "support_form": "any_support"},
    ]
...
def evaluate_metrics(
    ...
    result: dict[str, Any] = {"removed_nodes_count": len(removed_nodes)}
    ...
    primary_h1 = f"h1_{context.h1_primary_key}"
    primary_support = f"support_total_{context.h1_primary_key}"
    result["h1_reach_loss"] = float(result.get(primary_h1, np.nan))
    result["support_total"] = float(result.get(primary_support, np.nan))
...
def main() -> None:
    ...
    primary_view = str(args.view or m6_cfg.get("primary_view", "full"))
    ...
    interdiction_h1_keys = [str(x) for x in m6_cfg.get("interdiction_h1_keys", [h1_primary_key])]
    interdiction_h1_keys = [
        key
        for key in interdiction_h1_keys
        if any(f"{p['weight_mode']}_{p['support_form']}" == key for p in h1_profiles)
    ]
    if not interdiction_h1_keys:
        interdiction_h1_keys = [h1_primary_key]
    geodesic_tol = float(
    ...
    weight_table, weight_diag = load_prime_weight_table(
        weights_path=weights_path, prime_uids=prime_uids
    )
    ...
    baseline_metrics["h1_primary_key"] = h1_primary_key
    baseline_metrics["h1_profiles"] = [
        f"{p['weight_mode']}_{p['support_form']}" for p in h1_profiles
    ]
    baseline_metrics["h1_baseline_totals"] = h1_baseline_total
    ...
    h1_cols = [
        c for c in robustness_df.columns if c.startswith("h1_") and c not in {"h1_reach_loss"}
    ]
    h1_profile_rows: list[dict[str, Any]] = []
    for (strategy, frac, k_removed), sub in robustness_df.groupby(
        ["strategy", "frac_removed", "k_removed"], dropna=False
    ):
        row = {
        ...
        for col in h1_cols:
            ...
            row[f"{col}_p05"] = float(sub[col].quantile(0.05))
            row[f"{col}_p95"] = float(sub[col].quantile(0.95))
        h1_profile_rows.append(row)
    robustness_h1_profiles_df = pd.DataFrame(h1_profile_rows).sort_values(
        ["frac_removed", "strategy"], ascending=[True, True]
    )
    robustness_h1_profiles_out = m6_dir / f"robustness_summary_h1_profiles_{primary_view}.csv"
    robustness_h1_profiles_df.to_csv(robustness_h1_profiles_out, index=False)
    ...
    profile_contrast_rows: list[dict[str, Any]] = []
    for frac in removal_fracs:
        sub_frac = robustness_df[robustness_df["frac_removed"] == frac]
        rand = sub_frac[sub_frac["strategy"] == "random"]
        if rand.empty:
            continue
        for strategy in sorted(set(sub_frac["strategy"].tolist()) - {"random"}):
            tar = sub_frac[sub_frac["strategy"] == strategy]
            if tar.empty:
                continue
            row = {"view": primary_view, "strategy": strategy, "frac_removed": float(frac)}
            for col in ["h1_reach_loss", *h1_cols]:
                target_mean = float(tar[col].mean())
                random_mean = float(rand[col].mean())
                ratio = (
                    float(target_mean / random_mean)
                    if np.isfinite(random_mean) and random_mean > 0
                    else np.nan
                )
                row[f"{col}_target_mean"] = target_mean
                row[f"{col}_random_mean"] = random_mean
                row[f"{col}_target_vs_random_ratio"] = ratio
            profile_contrast_rows.append(row)
    contrast_h1_profiles_df = pd.DataFrame(profile_contrast_rows)
    contrast_h1_profiles_out = (
        m6_dir / f"robustness_target_vs_random_h1_profiles_{primary_view}.csv"
    )
    contrast_h1_profiles_df.to_csv(contrast_h1_profiles_out, index=False)
        ...
        for step in range(1, k_value + 1):
            ...
            for cand in remaining:
                ...
                if objective == "delay_h2":
                    value = float(score["h2_path_growth"])
                elif objective.startswith("deny_h1_"):
                    h1_key = objective.replace("deny_h1_", "", 1)
                    value = float(score.get(f"h1_{h1_key}", np.nan))
                else:
                    value = float(score.get("h1_reach_loss", np.nan))
                if np.isnan(value):
                    value = -np.inf
                if value > best_value:
                    best_value = value
    ...
    for k_value in interdiction_k:
        for h1_key in interdiction_h1_keys:
            interdiction_rows.extend(greedy_interdiction(f"deny_h1_{h1_key}", k_value))
    ...
    run_metadata = {
        ...
        "settings": {
            ...
            "h1_profiles": h1_profiles,
            "h1_primary_key": h1_primary_key,
            "interdiction_h1_keys": interdiction_h1_keys,
    ...
    manifest = {
        ...
        "outputs": {
            ...
            "robustness_summary_h1_profiles": str(robustness_h1_profiles_out),
            "robustness_target_vs_random": str(contrast_out),
            "robustness_target_vs_random_h1_profiles": str(contrast_h1_profiles_out),
            "interdiction_sets": str(interdiction_sets_out),
            "interdiction_performance": str(final_perf_out),
            "run_metadata": str(run_metadata_out),
        },
    }
    manifest_out = m6_dir / f"manifest_m6_1_{primary_view}.json"
    ...
    print(f"[done] wrote {robustness_h1_profiles_out}")
    print(f"[done] wrote {contrast_out}")
    print(f"[done] wrote {contrast_h1_profiles_out}")
